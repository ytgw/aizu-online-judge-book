...
public:
    ...
    ContainRelation calcPolygonRelation(const Point p) const {
        const int pointNum = points.size();
        int intersectNum = 0;
        for (int i = 0; i < pointNum; i++) {
            const Point p0 = points[i % pointNum];
            const Point p1 = points[(i+1) % pointNum];

            if (calcRelation(Segment(p0, p1), p) == SegmentPointRelation::ON_SEGMENT) {
                return ContainRelation::ON_SEGMENT;
            }

            double minY, maxY, crossProduct;
            if (p0.y < p1.y) {
                minY = p0.y - p.y;
                maxY = p1.y - p.y;
                crossProduct = calcCrossProduct(p0-p, p1-p);
            } else {
                minY = p1.y - p.y;
                maxY = p0.y - p.y;
                crossProduct = calcCrossProduct(p1-p, p0-p);
            }

            if (minY < EPS && EPS < maxY && crossProduct > EPS) intersectNum++;
        }

        if (intersectNum % 2 == 0) {
            return ContainRelation::NOT_CONTAIN;
        } else {
            return ContainRelation::CONTAIN;
        }
    }
};


std::vector<Point> andrewScan(const std::vector<Point> points) {
    ...
    upperPoints.push_back(sortedPoints[0]);
    upperPoints.push_back(sortedPoints[1]);
    for (int i = 2; i < sortedPoints.size(); i++) {
        const Point point = sortedPoints[i];
        while (upperPoints.size() >= 2) {
            const Segment segment = Segment(upperPoints[upperPoints.size()-2], upperPoints[upperPoints.size()-1]);
            const auto relation = calcRelation(segment, point);
            const bool isConvex = (relation == SegmentPointRelation::CLOCKWISE || relation == SegmentPointRelation::ONLINE_FRONT);
    ...
    const int pointNum = sortedPoints.size();
    lowerPoints.push_back(sortedPoints[pointNum - 1]);
    lowerPoints.push_back(sortedPoints[pointNum - 2]);
    for (int i = pointNum - 3; i >= 0; i--) {
        const Point point = sortedPoints[i];
        while (lowerPoints.size() >= 2) {
            const Segment segment = Segment(lowerPoints[lowerPoints.size()-2], lowerPoints[lowerPoints.size()-1]);
            const auto relation = calcRelation(segment, point);
            const bool isConvex = (relation == SegmentPointRelation::CLOCKWISE || relation == SegmentPointRelation::ONLINE_FRONT);
        ...
        lowerPoints.push_back(point);
    }

    // 戻り地の生成
    std::vector<Point> result;
    for (int i = lowerPoints.size() - 1; i >= 1; i--) result.push_back(lowerPoints[i]);
    for (int i = upperPoints.size() - 1; i >= 1; i--) result.push_back(upperPoints[i]);
...
int main() {
    ...
    for (int _ = 0; _ < pointNum; _++) {
        double x, y;
        std::cin >> x >> y;
        points.push_back(Point(x, y));
    }

    const auto convexHullPoints = andrewScan(points);
