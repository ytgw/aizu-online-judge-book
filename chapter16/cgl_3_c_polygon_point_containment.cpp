...
int main() {
    ...
    const Polygon polygon = Polygon(polygonPoints);
    ...
    for (int i = 0; i < queryNum; i++) {
        ...
        const auto relation = polygon.calcPolygonRelation(Point(x, y));
        if (relation == ContainRelation::CONTAIN) {
            std::cout << 2 << std::endl;
        } else if (relation == ContainRelation::ON_SEGMENT) {
            std::cout << 1 << std::endl;
        } else {
            std::cout << 0 << std::endl;
        }
